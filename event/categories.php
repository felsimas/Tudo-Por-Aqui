	...
	if ($categories) {
		...
		for ($i=0; $i<count($categories); $i++) {
			...
			if (MODREWRITE_FEATURE == "on") {
				$code_include .= "<h3><a href=\"".EVENT_DEFAULT_URL."/guide/".$categories[$i]["friendly_url".$langIndex]."\">".$categories[$i]["title".$langIndex]."</a></h3>";
			} else {
			...
			if (EVENTCATEGORY_SCALABILITY_OPTIMIZATION != "on") {
				...
				if ($subcategories) {
					...
					for ($j=0; $j<count($subcategories); $j++) {
						...
						if (MODREWRITE_FEATURE == "on") {
							$code_include_aux[$j] = "<a href=\"".EVENT_DEFAULT_URL."/guide/".$categories[$i]["friendly_url".$langIndex]."/".$subcategories[$j]["friendly_url".$langIndex]."\">".$subcategories[$j]["title".$langIndex]."</a>";
						} else {
