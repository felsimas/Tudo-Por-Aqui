<?
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (validate_form("listingsettings", $_POST, $message_listingsettings)) {
            ...
            //echo DEFAULT_URL."/gerenciamento/estabelecimentos/".(($search_page) ? "search.php" : "index.php")."?message=".urlencode($message)."&screen=$screen&letra=$letra".(($url_search_params) ? "&$url_search_params" : "")."";
			header("Location: ".DEFAULT_URL."/gerenciamento/estabelecimentos/".(($search_page) ? "search.php" : "index.php")."?message=".urlencode($message)."&screen=$screen&letra=$letra".(($url_search_params) ? "&$url_search_params" : "")."");
			exit;
...
?>
	...
	<div id="page-wrapper">
		...
		<?php 	include(SM_EDIRECTORY_ROOT."/menu.php"); ?>
			...
			<form id="formlistingsettingscancel" action="<?=DEFAULT_URL?>/gerenciamento/estabelecimentos/<?=(($search_page) ? "search.php" : "index.php");?>" method="post">
							<?=system_getFormInputSearchParams((($_POST)?($_POST):($_GET)));?>
							<input type="hidden" name="letra" value="<?=$letra?>" />
							<input type="hidden" name="screen" value="<?=$screen?>" />
			</form>
