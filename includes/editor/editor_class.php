...
class wysiwygPro {
	...
	*/
	function wp_entities($code) {
		// convert anchors:
		$code = preg_replace("/<a name=\"(.*?)\".*?>(.*?)<\/a>/smi", "<img name=\"\$1\" src=\"".WP_WEB_DIRECTORY."images/bookmark_symbol.gif\" alt=\"bookmark\" title=\"bookmark\" contenteditable=\"false\" width=\"16\" height=\"13\" />\$2", $code);
	...
	*/
	...
	function addspacer($title, $location) {
		if ($this->has_expired) {
			$button = '<td><img class="wpSpacer" width="1" height="22" src="'.WP_WEB_DIRECTORY.'images/spacer.gif" alt="" title="" /></td>';
			if (!empty($title)) {
				$before = "\n<!-- begin $title -->\n";
				$after = "\n<!-- end $title -->\n";
			} else {
