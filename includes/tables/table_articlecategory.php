			...
			if ($category_id) {
				...
				if ($path_elem_array) {
					foreach ($path_elem_array as $each_category) {
						echo " <a href=\"".$url_redirect."/index.php?category_id=".$each_category["id"]."&screen=".$screen."&letra=".$letra.(($url_search_params) ? "&$url_search_params" : "")."\">&raquo; ".$each_category["title"]."</a>";						$path_count++;
					}
				}
			}
			?>
...
<a href="<?=DEFAULT_URL?>/gerenciamento/articlecategs/category.php?category_id=<?=$category_id?>&screen=<?=$screen?>&letra=<?=$letra?><?=(($url_search_params) ? "&$url_search_params" : "")?>" class="addLINK"><?=system_showText(LANG_SITEMGR_CATEGORY_ADDNEW)?> &raquo;</a>
