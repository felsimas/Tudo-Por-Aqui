...
//echo "SELECT Gallery.* FROM Gallery inner join Gallery_Item on Gallery_Item.gallery_id = Gallery.id  WHERE item_id = ".$item_id." ORDER BY title";
			//$sql = "DELETE FROM Gallery WHERE account_id = ".$item->getNumber('account_id');
			...
			$dbObj->query($sql);
			//echo "3";
			$sql = "INSERT INTO Gallery_Item (item_id, gallery_id, item_type) VALUES (".$item_id.", ".$insert_id.", 'listing')";
...
//		header("Location: ".$url_base."/gallery/images.php?id=".$gallery["id"]."&empresaid=".$empresa_id);
