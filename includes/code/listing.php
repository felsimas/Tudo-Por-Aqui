				...
				******************************************************/
				...
				if ($emailNotification) {
					if(!strpos($url_base, "/gerenciamento")){
						...
						$sitemgr_msg .= "
										<a href=\"".DEFAULT_URL."/gerenciamento/estabelecimentos/view.php?id=".$listing->getNumber("id")."\" target=\"_blank\">".DEFAULT_URL."/gerenciamento/estabelecimentos/view.php?id=".$listing->getNumber("id")."</a><br /><br />
									</div>
								</body>
							</html>";
						...
						if ($sitemgr_send_email == "on") {
							if ($sitemgr_emails[0]) {
								foreach ($sitemgr_emails as $sitemgr_email) {
									system_mail($sitemgr_email, "[".EDIRECTORY_TITLE."] ".ucwords(LISTING_FEATURE_NAME), $sitemgr_msg, EDIRECTORY_TITLE." <".$sitemgr_email.">", "text/html", '', '', $error);
								}
							}
						}
				...
				if ($process == "claim") {
					...
					header("Location: $url_redirect/".(($search_page) ? "search.php" : "index.php")."?process=".$process."&newest=".$newest."&message=".urlencode($message)."&extramessage_gallery=".$extramessage_gallery."&extramessage_promotion=".$extramessage_promotion."&screen=$screen&letra=$letra".(($url_search_params) ? "&$url_search_params" : ""));
