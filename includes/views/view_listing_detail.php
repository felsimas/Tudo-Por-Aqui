<?
	...
	if ($listing->getString("video_snippet")) {
		$listingtemplate_video_snippet_width  = IMAGE_LISTING_FULL_WIDTH + 20;
		$listingtemplate_video_snippet_height = IMAGE_LISTING_FULL_HEIGHT;
		$listingtemplate_video_snippet = system_getVideoSnippetCode($listing->getString("video_snippet", false), IMAGE_LISTING_FULL_WIDTH, IMAGE_LISTING_FULL_HEIGHT);
	}
	...
	if ($listing->getString("email")) {
		if ($user){
			$listingtemplate_email = "<a href=\"".LISTING_DEFAULT_URL."/detail.php?id=".$listing->getNumber("id")."#info\">".$listing->getString("email", true)."</a>";
		} else {
