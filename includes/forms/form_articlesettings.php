...
<div id="header-form"><?=ucwords(system_showText(LANG_SITEMGR_MODIFY))?> <?=system_showText(LANG_SITEMGR_ARTICLE_SING)?> <?=ucwords(system_showText(LANG_SITEMGR_MENU_SETTINGS))?> - <?=$articleObj->getString("title")?></div>

<? if ($message_articlesettings) { ?>
	<div class="response-msg error ui-corner-all"><?=$message_articlesettings?></div>
<? } ?>
	...
	<? if ($articleObj->hasRenewalDate()) { ?>
		<tr class="tr-form">
			...
			<td align="left" class="td-form">
				...
				<a href="javascript:void(0);" onclick="if (emptyDate(document.getElementById('renewal_date'))) { document.getElementById('renewal_date').value=''; } cal_renewal_date.popup()" title="<?=system_showText(LANG_SITEMGR_CLICKHERETOSELECTRENEWALDATE)?>"><img src="<?=DEFAULT_URL?>/images/calendar/cal.gif" alt="Calendar" title="<?=system_showText(LANG_SITEMGR_CLICKHERETOSELECTRENEWALDATE)?>" border="0" class="iconAlign" /></a>
				<?
		...
		<? if (PAYMENT_FEATURE == "on") { ?>
			<? if (MANUALPAYMENT_FEATURE == "on") { ?>
				...
				<div id="trans_form" class="base-table-form-account" style="display: <?=(($_POST["add_transaction"] == "1") ? "block" : "none" )?>;">
					...
					<?
					$acct_search_table_title = system_showText(LANG_SITEMGR_ACCOUNTSEARCH_SELECT)." ".system_showText(LANG_SITEMGR_ARTICLE_SING);
