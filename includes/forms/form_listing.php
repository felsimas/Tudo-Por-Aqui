...
<table cellpadding="0" cellspacing="0" border="0" class="standard-table">
	...
	<tr>
		<th style="width:200px" class="wrap">
			<?
			if (LISTINGTEMPLATE_FEATURE == "on") {
				echo "* ".(($template_title_field!==false) ? $template_title_field[0]["label"] : "".system_showText("Nome de Fantasia")).":";
			} else {
				echo "* ".system_showText("Nome de Fantasia:").":";
			}
			?>
		</th>
		<td>
	...
	<? if ($level > "10") { ?>
		<tr>
			...
			<td><input style="width:400px" type="text" name="razaosocial" value="<?=$razaosocial?>" maxlength="50" /></td>
		</tr>
        <tr>:
	...
	<tr>
		<th colspan="2" class="standard-tabletitle">
			...
			if (((!$listing->getNumber("id")) || (($listing) && ($listing->needToCheckOut())) || (strpos($url_base, "/gerenciamento")) || (($listing) && ($listing->getPrice() <= 0))) && ($process != "signup")) {
				...
				?><span><br>Seu plano mensal tem direito a escolher <?=$listingLevelObj->getFreeCategory($level)?> categorias. <br>Categorias extras ter&#227;o um custo adicional de <?=CURRENCY_SYMBOL?> 5,00.<br>
