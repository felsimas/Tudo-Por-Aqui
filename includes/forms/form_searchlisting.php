...
<table border="0" cellpadding="2" cellspacing="0" class="standard-table" style="margin-top: 20px;">
	<tr>
		<th>&nbsp;</th>
	...
	<tr>
		<th><?=ucwords(system_showText(LANG_SITEMGR_LISTING_LISTINGTITLE))?>:</th>
		<td><input type="text" name="search_title" value="<?=$search_title?>" class="input-form-searchlisting" /></td>
	</tr>

	<? if (!$user) { ?>
		<tr>
			<th><?=ucwords(system_showText(LANG_SITEMGR_LISTING_LISTINGLEVEL))?>:</th>
			<td>
	...
	<? if (LISTINGTEMPLATE_FEATURE == "on") { ?>
	  <!--	<tr>
			<th><?=ucwords(system_showText(LANG_SITEMGR_LISTING_LISTINGTEMPLATE))?>:</th>
