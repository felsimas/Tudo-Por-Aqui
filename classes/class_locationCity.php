<?
	...
	class LocationCity extends Handle {
		...
		function Save() {
			...
			if ($this->id) {
				...
				$sql = "INSERT INTO Location_City"
					. " (id, bairro_id, name, abbreviation, friendly_url, seo_description, seo_keywords)"
					. " VALUES"
		...
		function isRepeated(){
			...
			$dbObj = db_getDBObJect();
			$sql = "SELECT * FROM Location_City WHERE name = ".db_formatString($this->name)." AND bairro_id = $this->bairro_id";
		...
		function retrievedIfRepeated() {
			...
			$sql = "SELECT * FROM Location_City WHERE friendly_url = ".db_formatString($this->friendly_url)." AND bairro_id = $this->bairro_id";
			if ($this->id) $sql .= " AND id != $this->id";
		...
		function retrieveCitiesByRegion(){
			...
			$dbObj = db_getDBObJect();
			$sql = "SELECT * FROM Location_City WHERE bairro_id = $this->bairro_id ORDER BY name";
			$result = $dbObj->query($sql);
			while($row = mysql_fetch_assoc($result)) $data[] = $row;
			if($data) return $data; else return false;
		}
		...
		function isValidFriendlyUrl(&$error_message) {
			...
			if($this->getString("bairro_id"))
				$sql .= " AND bairro_id = $this->bairro_id ";
